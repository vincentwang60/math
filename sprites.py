...
class piece(pg.sprite.Sprite):
    def __init__(self, game,x,y,type):
        ...
        self.replaced_op = None #the operation type that is replaced by =
        if self.type == 0:
    ...
    def check_click(self,mouse_pos,click_dir):
        if self.rect.collidepoint(mouse_pos):
            if self.type == 2 and self.connected_from != None:#if end piece
                for i in range(4):
            ...
            if self.mask.get_at(pos_in_mask) == 1:
                self.holding = True
                self.connected_to = []
    ...
    def update(self):
        ...
        if self.rect.x < WIDTH-TILESIZE*2.5:
            ...
            self.on_board = False
        if self.type == 0 and self.on_board:
            for i in range(4):
                if self.sides[i] != False:
                    self.out_coords.append([self.text_quarter_pos[i][0]+self.rectx,self.text_quarter_pos[i][1]+self.recty])
                else:
                    self.in_coords.append([self.in_quarter_pos[i][0]+self.rectx,self.in_quarter_pos[i][1]+self.recty])
        elif self.type == 1:
            self.out_coords.append([self.text_quarter_pos[0][0]+self.rectx,self.text_quarter_pos[0][1]+self.recty])
        elif self.type == 2:
            self.in_coords.append([self.in_quarter_pos[2][0]+self.rectx,self.in_quarter_pos[2][1]+self.recty])
        if self.holding:
